...
import com.mohsen.game.application.persian.calendar.util.ConversionHelper;
import com.mohsen.game.application.persian.calendar.util.PersianCalendarConstants;
import com.mohsen.game.application.persian.calendar.util.PersianCalendarUtils;

import java.util.Calendar;
import java.util.GregorianCalendar;
import java.util.TimeZone;

import static com.mohsen.game.application.persian.calendar.util.StringHelper.convertNumbersInTextToPersian;
...
public class PersianCalendar extends GregorianCalendar {
	...
	private final long MinimumDate = 986021621743L;
	...
	public PersianCalendar() {
		...
		if (getTimeInMillis() < MinimumDate) {
			setTimeInMillis(MinimumDate);
		}
		
		this.setFirstDayOfWeek(SATURDAY);
		resetCalendarTime();
		
		
	}
	
	long getRawJulianOffset() {
		return PersianCalendarConstants.MILLIS_JULIAN_EPOCH - getTimeZoneOffset();
	}
	...
	public long getTimeZoneOffset() {
		...
		long timeZoneOffset = tz.getOffset(System.currentTimeMillis());

       /* if(tz.inDaylightTime(getTime()) == false)
            timeZoneOffset += tz.getDSTSavings();*/
		
		return timeZoneOffset;
		
	}
	
	public void resetCalendarTime() {
		int year = getPersianYear(), month = getPersianMonth(), day = getPersianDay();
	...
	public void setTime(int Hour, int Minutes, int Seconds, int Milliseconds, int AmPm) {
		...
		this.set(Calendar.AM_PM, AmPm);
		setPersianDate(year, month, day);
		
		
	}
	
	public void setTimeIn24Hour(int Hour, int Minutes, int Seconds, int Milliseconds) {
	...
	public String getHour() {
		return this.get(Calendar.HOUR_OF_DAY) < 10 ? "0" + this.get(Calendar.HOUR_OF_DAY) : this.get(Calendar.HOUR_OF_DAY) + "";
	}
	
	public String getMinute() {
		return this.get(Calendar.MINUTE) < 10 ? "0" + this.get(Calendar.MINUTE) : this.get(Calendar.MINUTE) + "";
	}
	
	public String getSecond() {
		return this.get(Calendar.SECOND) < 10 ? "0" + this.get(Calendar.SECOND) : this.get(Calendar.SECOND) + "";
	}
	
	public String getShortTime() {
		return convertNumbersInTextToPersian(this.getHour() + ":" + this.getMinute());
	}
	
	public String getLongTime() {
		return convertNumbersInTextToPersian(this.getHour() + ":" + this.getMinute() + ":" + this.getSecond());
	}
	
	public String getLongDatePlusTime() {
		String datePart = this.getPersianLongDate();
		String timePart = this.getShortTime();
		return datePart + " " + timePart;
	}
	
	public String getShortDatePlusTime() {
		String datePart = this.getPersianShortDate();
		String timePart = this.getShortTime();
		return this.getPersianDayNameOfWeek() + "  " + datePart + " " + timePart;
	}
	
	private long convertToMilis(long julianDate) {
		return getRawJulianOffset() + julianDate * PersianCalendarConstants.MILLIS_OF_A_DAY
				+ PersianCalendarUtils.ceil(getTimeInMillis() - getRawJulianOffset(), PersianCalendarConstants.MILLIS_OF_A_DAY);
	}
	...
	protected void calculatePersianDate() {
		...
		long millis = getTimeInMillis() - 0;
		
		long julianDate = ((long) Math.floor((millis - (getRawJulianOffset()))) /
	...
	public void setTimeInMillis(long millis) {
		...
		if (millis < MinimumDate) {
			setTimeInMillis(MinimumDate);
		}
		calculatePersianDate();
	}
	
	public void setPersianDate(String faDate) {
		
		String[] pDate = faDate.split("/");
		
		setPersianDate(
				ConversionHelper.StringToInteger(pDate[0])
				, ConversionHelper.StringToInteger(pDate[1])
				, ConversionHelper.StringToInteger(pDate[2]));
	}
	 ...
	 */
	public String getPersianDayNameOfWeek() {
		switch (get(DAY_OF_WEEK)) {
			case SATURDAY:
				return PersianCalendarConstants.persianWeekDayNames[0];
			case SUNDAY:
				return PersianCalendarConstants.persianWeekDayNames[1];
			case MONDAY:
				return PersianCalendarConstants.persianWeekDayNames[2];
			case TUESDAY:
				return PersianCalendarConstants.persianWeekDayNames[3];
			case WEDNESDAY:
				return PersianCalendarConstants.persianWeekDayNames[4];
			case THURSDAY:
				return PersianCalendarConstants.persianWeekDayNames[5];
			default:
				return PersianCalendarConstants.persianWeekDayNames[6];
		}
	...
	public String getPersianLongDate() {
		return convertNumbersInTextToPersian(getPersianDayNameOfWeek() + "  " + formatToMilitary(this.persianDay) + "  "
	...
	public String getPersianShortDate() {
		//calculatePersianDate();
		return convertNumbersInTextToPersian("" + formatToMilitary(this.persianYear) + delimiter + formatToMilitary(getPersianMonth()) + delimiter
				+ formatToMilitary(this.persianDay));
	}
	
	public String getPersianShortDatePlusTime() {
		return getPersianDayNameOfWeek() + getPersianShortDate();
	}
	...
	public void addPersianDate(int field, int amount) {
		...
		if (field == YEAR) {
			...
			int newMonthValue = ((getPersianMonth() + amount) % 12 == 0) ? 12 : (getPersianMonth() + amount) % 12;
			
			setPersianDate(this.persianYear + ((this.persianMonth + amount) / 12), newMonthValue, this.persianDay);
			calculatePersianDate();
			return;
		}
		add(field, amount);
	...
	public int getPersianMonthDaysCount() {
		if (getPersianMonth() < 7)
			return 31;
		if (getPersianMonth() >= 7 && getPersianMonth() <= 11)
			return 30;
		if (getPersianMonth() == 12 && isPersianLeapYear())
			return 30;
		else
			return 29;
	}
	
	public String getRawPersianDate() {
		String month = persianMonth < 10 ? "0" + persianMonth : persianMonth + "";
		String day = persianDay < 10 ? "0" + persianDay : persianDay + "";
